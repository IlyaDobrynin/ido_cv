...
class MultiTverskyLoss(nn.Module):
    ...
    def make_loss(self, preds: torch.Tensor, trues: torch.Tensor):
        num_class = preds.size(1)
        weight_losses = 0.0
        if self.weights is not None:
            assert len(self.weights) == num_class, \
                'number of classes should be equal to length of weights '
            weights = self.weights
        else:
            weights = [1.0 / num_class] * num_class
        input_slices = torch.split(preds, [1] * num_class, dim=1)
        for idx in range(num_class):
            input_idx = input_slices[idx]
            input_idx = torch.cat((1 - input_idx, input_idx), dim=1)
            target_idx = (trues == idx) * 1
    ...
    def forward(self, preds: torch.Tensor, trues: torch.Tensor):
        ...
        loss = self.make_loss(preds, trues)
        return loss
