...
class FocalTverskyBase(Function):
    ...
    def forward(ctx, preds: torch.Tensor, trues: torch.Tensor):
        batch_size = preds.size(0)
        _, input_label = preds.max(1)

        input_label = input_label.float()
        target_label = trues.float()

        ctx.save_for_backward(preds, target_label)
