...
def get_wrn(
    ...
    if pretrained == 'imagenet':
        if (model_name is None) or (not model_name):
            raise ValueError(
                "Parameter `model_name` should be properly "
                "initialized for loading pretrained model."
            )
        download_model(
            net=model,
        ...
        raise ValueError(
            f"Wrong pretrained parameter: {pretrained}. "
            f"Should be 'imagenet'."
        )
...
if __name__ == '__main__':
    ...
    model = wrn50_2(pretrained='imagenet')
    ...
    encoder = WRNEncoder(model, layers=wrn50_2_layers)
    import torch
